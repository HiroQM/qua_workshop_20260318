...
def build_program(config: dict[str, object]) -> Any:
    ...
    with program() as qua_program:
        n = declare(int)
        with for_(n, 0, n < CHIRP_REPETITIONS, n + 1):
            reset_if_phase(LFM1_ELEMENT)
            reset_if_phase(LFM2_ELEMENT)
            update_frequency(LFM1_ELEMENT, CHIRP_START_HZ)
            update_frequency(LFM2_ELEMENT, CHIRP_START_HZ)
            frame_rotation_2pi(PHASE_SHIFT_TURNS, LFM2_ELEMENT)
            align(LFM1_ELEMENT, LFM2_ELEMENT)
            play(
                "const",
                LFM1_ELEMENT,
            ...
            wait(GAP_DURATION_CLOCKS, LFM1_ELEMENT, LFM2_ELEMENT)
            reset_frame(LFM1_ELEMENT, LFM2_ELEMENT)
